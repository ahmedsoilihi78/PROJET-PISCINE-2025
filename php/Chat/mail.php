<?php
// demarre la session pour recuperer les informations de l utilisateur connecte
session_start();
// inclut le fichier de connexion a la base de donnees pour pouvoir utiliser la variable pdo
require_once __DIR__ . '/../connexion.php';

// verifie si l utilisateur n est pas connecte, sinon redirige vers la page de connexion
if (!isset($_SESSION['user_id'])) {
    header("Location: /../connexion.php");
    exit();
}
// recupere l id de l utilisateur actuel depuis la session
$current_user_id = $_SESSION['user_id'];

// recupere le role de l utilisateur actuel depuis la base de donnees
$stmt = $pdo->prepare("SELECT role FROM users WHERE id = ?");
$stmt->execute([$current_user_id]);
$current_user = $stmt->fetch();
// stocke le role ou chaine vide si impossible de recuperer
$current_role = $current_user ? $current_user['role'] : '';

// si l utilisateur est de role client, on affiche seulement les utilisateurs non clients
if ($current_role === 'client') {
    // prepare la requete pour recuperer les utilisateurs qui ne sont pas le client actuel et qui ne sont pas de role client
    $stmt = $pdo->prepare("SELECT id, nom, prenom, email, role FROM users WHERE id != ? AND role != 'client'");
    $stmt->execute([$current_user_id]);
} else {
    // si l utilisateur est admin ou coach, on affiche tous les utilisateurs sauf lui-meme
    $stmt = $pdo->prepare("SELECT id, nom, prenom, email, role FROM users WHERE id != ?");
    $stmt->execute([$current_user_id]);
}
// recupere la liste des utilisateurs selon la requete precedente
$users = $stmt->fetchAll();

// initialise le corps du mail avec un message de bienvenue
$body = "bonjour,\n\n";
?>
<!DOCTYPE html>
<html lang="fr">
<head>
    ...
    <!-- inclusion des fichiers css de base et du style pour la page de mail -->
...
<div class="wrapper">
    ...
    <!-- HEADER avec titre et logo du site -->
    <header class="header">
        <div class="title">
            <!-- titre principal avec classes pour la couleur -->
    ...
    <!-- NAVIGATION principale avec les boutons de redirection -->
    <nav class="navigation">
        ...
        <!-- bouton vers la page de gestion du compte -->
    ...
    <div id="rechercheContainer" class="recherche-form">
        <form method="get" action="../../html/recherche/barre_recherche.html">
            <!-- champ texte pour saisir la requete -->
            <input type="text" name="q" placeholder="rechercher..." />
            <button type="submit">rechercher</button>
        </form>
    </div>

    <!-- SECTION PRINCIPALE pour l envoi de mail -->
    <section class="main-section">
        <h2>envoyer un mail a un utilisateur</h2>
        <div class="users-list">
            <!-- boucle sur chaque utilisateur pour afficher un bloc de destinataire -->
            <?php foreach ($users as $user):
                // genere l url pour ouvrir outlook web avec le destinataire et le corps du message
              ...
              <span class="role">(<?= $user['role'] ?>)</span>
                    ...
                    <div class="buttons">
                        <!-- bouton pour ouvrir outlook web dans un nouvel onglet -->
    ...
    <!-- FOOTER avec informations de contact et carte google maps -->
    <footer class="footer">
        ...
        <div class="map">
            <!-- zone pour afficher la carte google maps integree -->
            <iframe
